...
namespace Spells
{
    public class TalonShadowAssault : ISpellScript
    {
        ...
        public void OnSpellPostCast(Spell spell) { spell.SetCooldown(0.25f, true); }
    ...
    /// <summary>
    /// Talon R - Shadow Assault Outgoing Blade
    /// Damage: 120/170/220 (+75% bonus AD)
    /// </summary>
    public class TalonShadowAssaultMisOne : ISpellScript
    {
        ...
        public void TargetExecute(Spell spell, AttackableUnit target, SpellMissile missile, SpellSector sector)
        {
            ...
            // Damage: 120/170/220 (+75% bonus AD)
            float[] baseDamage = { 120f, 170f, 220f };
            float damage = baseDamage[owner.GetSpell("TalonCutthroat").CastInfo.SpellLevel - 1] + (owner.Stats.AttackDamage.FlatBonus * 0.75f);

            if (target.HasBuff("TalonDamageAmp"))
            {
                damage *= 1f + (0.03f * owner.GetSpell("TalonCutthroat").CastInfo.SpellLevel);
            }

            if (!UnitsHit.Contains(target) && target != owner && target.Team != owner.Team && !(target is ObjBuilding || target is BaseTurret))
            {
                UnitsHit.Add(target);
    ...
    /// <summary>
    /// Talon R - Shadow Assault Return Blade
    /// Damage: 120/170/220 (+75% bonus AD)
    /// </summary>
